# Smart Greenhouse IoT - ESP32 Code (Fixed Display Logic)
...
const int SERVO_OPEN = 60;   // Sudut 60 derajat = Jendela TERBUKA
...
void sendDataToServer() {
    ...
    // PERBAIKAN: Logika status jendela dibalik agar sesuai tampilan web
    // windowOpen = true (TERBUKA) -> kirim 1 -> Web: "TERTUTUP"
    // windowOpen = false (TERTUTUP) -> kirim 0 -> Web: "TERBUKA"
...
void manualControl() {
    if (webWindowCmd == 1 && !windowOpen) openWindow();    // cmd 1 = BUKA
    else if (webWindowCmd == 0 && windowOpen) closeWindow(); // cmd 0 = TUTUP
...
void openWindow() {
    ...
    myServo.attach(SERVO_PIN);
    delay(15);
    myServo.write(SERVO_OPEN);   // 60 derajat = TERBUKA
    delay(500);
    myServo.detach();
    windowOpen = true;
}
...
**HANYA 2 BARIS YANG DIUBAH:**

1. **Di `sendDataToServer()` - Line pengiriman data:**
   ```cpp
   // DIBALIK dari yang sebelumnya:
   url += "&window=" + String(windowOpen ? 1 : 0);
   ```

2. **Di `manualControl()` - Logika kontrol:**
   ```cpp
   if (webWindowCmd == 1 && !windowOpen) openWindow();    // cmd 1 = BUKA
   else if (webWindowCmd == 0 && windowOpen) closeWindow(); // cmd 0 = TUTUP
   ```
...
4. Ganti WiFi dan Server URL di kode atas (jika perlu)
